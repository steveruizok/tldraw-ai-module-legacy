import { type TLAiChange, type TLAiPrompt, TldrawAiTransform } from '../ai'
import { createBindingId, createShapeId } from 'tldraw'
...
export class SimpleIds extends TldrawAiTransform {
	...
	override transformChange = (change: TLAiChange): TLAiChange => {
		switch (change.type) {
			...
			case 'createBinding': {
				...
				binding = this.collectAllIdsRecursively(
					change.binding,
					this.writeOriginalIds
				)
			...
			case 'updateBinding': {
				const binding = this.collectAllIdsRecursively(
					change.binding,
					this.writeOriginalIds
				)
	...
	private mapObjectWithIdAndWriteSimple = (obj: {
		id: string
		fromId?: string
		toId?: string
	}) => {
