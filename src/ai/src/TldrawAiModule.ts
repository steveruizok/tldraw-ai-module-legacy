...
import { type TldrawAiTransformConstructor } from './TldrawAiTransform'
import {
	type TLAiChange,
	type TLAiContent,
	type TLAiMessages,
...
export class TldrawAiModule {
	private opts: TldrawAiModuleOptions
	...
	async getPrompt(
		prompt: TLAiMessages,
		options = {} as Partial<
			Pick<TLAiPrompt, 'canvasContent' | 'contextBounds' | 'promptBounds'>
		>
	): Promise<TLAiPrompt> {
		const { editor } = this.opts
		const {
	...
	private getContent(bounds: Box): TLAiContent {
		...
		if (content) {
			...
			delete (content as unknown as { schema: unknown }).schema
			delete (content as unknown as { rootShapeIds: unknown }).rootShapeIds
		} else {
			content = {
				shapes: [],
				bindings: [],
