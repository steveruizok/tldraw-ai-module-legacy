import { type TLAiContent } from '../../../src/ai'
...
function toPlainText(richText: JSONContent): string {
	return generateText(richText, tipTapDefaultExtensions, {
		blockSeparator: '\n',
	})
}
			...
			content.shapes.map((shape) => {
				if (shape.type === 'text') {
					...
					return {
						shapeId: s.id,
						type: 'text',
						text: toPlainText(s.props.richText as JSONContent),
						x: s.x,
						y: s.y,
						color: s.props.color,
				...
				if (shape.type === 'arrow') {
					...
					const startBinding = arrowBindings.find(
						(b) => b.props.terminal === 'start'
					)
					const endBinding = arrowBindings.find(
						(b) => b.props.terminal === 'end'
					)
